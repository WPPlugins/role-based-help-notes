...
class RBHN_Capabilities {
	...
	public function rbhn_map_meta_cap( $caps, $cap, $user_id, $args ) {
        ...
        public function rbhn_attachment_map_meta_cap( $caps, $cap, $user_id, $args ) {
            ...
            if ( 'edit_post' == $cap || 'delete_post' == $cap || 'read_post' == $cap ) {
                ...
                if ( 'attachment' == $post->post_type ) {
                     ...
                     * this is actually never used by Wordpress's capabilities for attachments as edit_posts is used.
        ...
        public function rbhn_posts_where(  $where, $object  ){
            ...
            if( ! is_admin( )                                                           // drop out if on front of site
                OR ( strpos( $where, 'attachment' ) === false )                         // drop out if not an attachment post type query.
                OR ( $pagenow !== 'upload.php' && $pagenow !== 'admin-ajax.php' )       // or currenly not on either page for 'admin-ajax.php' or admin-ajax.php ( used for 'upload.php?mode=grid')
                OR ( ! current_user_can( 'upload_files' ) )                             // or current user has neither the standard WP upload_file cap nor the Help Notes upload_file meta cap added.
            ) {   
                return $where;
            }
            ...
            // and so create an infinite loop
            ...
            if ( $user_has_upload_files_wp_cap ) {
                ...
                if ( $helpnote_parent_not_available ) {
        ...
        public function rbhn_limit_current_user_attachments(  $query = array()  ){
             ...
             * 'rbhn_posts_where' hooked function will fire and restrict the media seen by these users.
             * 
             * remove upload_files map_meta_cap hook so that 'upload_files' capability relates to the 
             * user allocated caps before HelpNotes adds the cap to users.
             */
            remove_filter( 'map_meta_cap', array( $this, 'rbhn_upload_file_map_meta_cap' ), 10, 4);
